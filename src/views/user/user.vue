  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="用户等级" prop="userLevel">
        <template slot-scope="scope">
          <el-tag :type="scope.row.userLevel == 0 ?'primary':'success'">{{ levelDic[scope.row.userLevel] }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button type="warning" size="mini" @click="bindSalesman(scope.row)">绑定业务员</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 绑定业务员对话框 -->
    <el-dialog :visible.sync="dialogSalesmanVisible" title="绑定业务员">
      <el-form :model="salesmanForm" label-width="80px">
        <el-form-item label="业务员手机号">
          <el-input v-model="salesmanForm.name" placeholder="请输入业务员手机号" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogSalesmanVisible = false">取消</el-button>
        <el-button type="primary" @click="submitBindSalesman">确认</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import { fetchList, userDetail, updateUser,bindSalesman } from '@/api/user'
  ...
  methods: {
    ...
    handleUserUpdate() {
      updateUser(this.userDetail).then(() => {
        ...
        this.$notify.success({ title: '成功', message: '更新用户成功' })
        this.getList()
      })
    },
    ...
    submitBindSalesman() {
        const mobile = this.salesmanForm.name.trim()  // 获取手机号
        if (!mobile) {
          this.$message.warning('请输入业务员手机号')
          return
        }

        const payload = {
          id: this.selectedUserId,  // 使用 this.selectedUserId 获取用户ID
          mobile: mobile  // 使用获取的 mobile
        }

        // 调用 GET 请求
        bindSalesman(payload).then(() => {

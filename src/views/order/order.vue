<template>
  <div class="app-container">
    ...
    <el-tabs v-model="listQuery.showType" type="card" @tab-click="handleClick">
      ...
      <el-tab-pane :label="`已签单 (${info.n6})`" name="6"></el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    confirmShip() {
  // 从 channels 中找到匹配当前 shipChannel 的项
  const selectedChannel = this.channels.find(
    channel => channel.code === this.shipForm.shipChannel
  )

  // 如果找到了匹配项，则将其 name 赋值为 shipSn
  if (selectedChannel) {
    this.shipForm.shipSn = selectedChannel.name
  }
